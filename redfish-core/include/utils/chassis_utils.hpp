    ...
    namespace redfish
{
    ...
    inline AllowListMap getRoTChassisAllowListMap()
    {
        ...
        allowListMap =
            data.get<std::map<std::string, std::vector<std::string>>>();
        return allowListMap;
    }

    inline void resetPowerLimit(
    ...
    inline std::string getFeatureReadyStateType(const std::string& stateType)
    {
        ...
        if (stateType ==
            "xyz.openbmc_project.State.FeatureReady.States.Disabled")
        {
            return "Disabled";
        }
        if (stateType ==
    ...
    {
        if (state ==
            "com.nvidia.StaticPowerHint.StateOfEstimatePower.Completed")
        {
            return "Completed";
        }
        if (state ==
            "com.nvidia.StaticPowerHint.StateOfEstimatePower.InProgress")
        {
            return "InProgress";
        }
        ...
        auto respHandler =
            [callback{std::forward<Callback>(callback)}, asyncResp,
             chassisID](const boost::system::error_code& ec,
                ...
                {
                    BMCWEB_LOG_ERROR(
                        "getValidChassisID respHandler DBUS error: {}", ec);
                    messages::internalError(asyncResp->res);
                    return;
                }
                    ...
                    crow::connections::systemBus->async_method_call(
                        [&req, asyncResp, chassisUUID, serviceName, option,
                         enabled, chassisId, chassisProcessed](
                            ...
                            const std::vector<uint8_t>* supportedMsgTypes =
                                nullptr;
                            bool foundEID = false;
    ...
    inline void setBackgroundCopyEnabled(
        const crow::Request& req,
        const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
        const std::string& chassisId, const std::string& chassisUUID,
        bool enabled)
    {
    ...
    {
        handleMctpInBandActions(req, asyncResp, chassisUUID,
                                InBandOption::setInBandEnabled, enabled,
                                chassisId);
    }
    ...
    inline void isEROTChassis(const std::string& chassisID,
        ...
        crow::connections::systemBus->async_method_call(
            [invObjPath, callback](const boost::system::error_code& ec,
                ...
                for (const auto& serObj : resp)
                {
                    ...
                    BMCWEB_LOG_DEBUG(
                        "Not found proper interface for service {}", service);
                }
