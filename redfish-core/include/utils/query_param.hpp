    ...
    namespace redfish
{
    ...
    inline Query delegate(const QueryCapabilities& queryCapabilities,
                          Query& query)
    {
        Query delegated{};
        // delegate only
        ...
        if (query.expandType != ExpandType::None)
        {
            ...
            else
            {
                delegated.expandLevel =
                    queryCapabilities.canDelegateExpandLevel;
            }
        }
    ...
    inline std::optional<Query> parseParameters(
        boost::urls::params_view urlParams, crow::Response& res)
    {
        Query ret{};
        ...
        bool inLinks, std::vector<ExpandNode>& out)
    {
        ...
        if (array != nullptr)
        {
            findNavigationReferencesInArrayRecursive(
                eType, *array, jsonPtr, depth, skipDepth, inLinks, out);
        }
    ...
    inline std::vector<ExpandNode> findNavigationReferences(
        ExpandType eType, int depth, int skipDepth,
...
>>>>>>> origin/master
        {
            ...
            const std::optional<std::string> queryStr =
        ...
        if (object != nullptr)
        {
            ...
            while (it != object->end())
            {
                ...
                constexpr std::array<std::string_view, 5> reservedProperties = {
                    ...
                    "error"};
                bool reserved =
                    std::ranges::find(reservedProperties, it->first) !=
...
>>>>>>> origin/master
    {
        ...
        if (query.filter)
        {
            applyFilterToCollection(intermediateResponse.jsonValue,
                                    *query.filter);
        }
