 ...
 * @return void
 */
inline void getFwRecoveryStatus(
    ...
    const std::shared_ptr<std::string>& swId, const std::string& dbusSvc)
{
    ...
    dbus::utility::getAllProperties(
        ...
        [asyncResp, swId, getLastSegnmentFromDotterString](
            ...
            const auto& it =
                std::ranges::find_if(propertiesList, [](const auto& property) {
                    return property.first == "Health";
                });
            if (it == propertiesList.end())
            {
                BMCWEB_LOG_ERROR(
    ...
    const std::string& swId, const std::string& dbusSvc)
{
    ...
    dbus::utility::getAllProperties(
        dbusSvc, swId, "xyz.openbmc_project.Software.Settings",
        [asyncResp,
         swId](const boost::system::error_code errorCode,
            ...
            auto it =
                std::ranges::find_if(propertiesList, [](const auto& property) {
                    return property.first == "WriteProtected";
                });
            if (it == propertiesList.end())
            {
...
{
    sdbusplus::object_path activeSlotAssoc =
        sdbusplus::object_path(swObjPath) / "ActiveSlot";
