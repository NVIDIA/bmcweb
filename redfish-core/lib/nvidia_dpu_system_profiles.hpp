...
inline std::string
    actionStatusToString(nvidia_system_profile::ActionStatus status)
{
    switch (status)
    {
...
inline nvidia_system_profile::ActionStatus
    getStatusActionFromDbusStatus(std::string_view status)
{
 ...
 */
inline void
    handleGetProfilesCollection(crow::App& app, const crow::Request& req,
...
inline void
    getProfileStatusInfo(const boost::system::error_code& ec,
                         ...
                         const std::string& profileNumber)
{
    ...
    auto handleAddStatus = [aResp](const std::string& status,
...
inline void
    getProfileFileInfo(const boost::system::error_code& ec,
                       const dbus::utility::DBusPropertiesMap& properties,
                       const std::shared_ptr<bmcweb::AsyncResp>& aResp,
                       const std::string& profileNumber)
{
...
inline void getProfileInfoIndex(const std::shared_ptr<bmcweb::AsyncResp>& aResp,
                                int profileIndex,
                                const boost::system::error_code& ec,
                                const std::string& property)
{
...
void handleProfileUpdateCall(task::Payload&& payload,
                             const boost::system::error_code& ec,
                             const std::shared_ptr<bmcweb::AsyncResp>& aResp,
                             bool isBiosUser, std::shared_ptr<MemoryFD> memFd)
{
    ...
    crow::connections::systemBus->async_method_call(
        ...
        callbackProfileUpdate(ec, msg, std::move(payload), aResp, isBiosUser,
                              profileNumber);
...
inline bool
    handleFactoryResetTask(boost::system::error_code ec,
                           sdbusplus::message_t& msg,
                           const std::shared_ptr<task::TaskData>& taskData)
{
    if (ec)
 ...
 */
inline void
    handlePatchProfilesStatus(crow::App& app, const crow::Request& req,
                              const std::shared_ptr<bmcweb::AsyncResp>& aResp,
                              const std::string& systemName)
{
    if (!redfish::setUpRedfishRoute(app, req, aResp))
