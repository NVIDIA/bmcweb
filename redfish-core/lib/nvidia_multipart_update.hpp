...
inline bool parseRfaUri(std::string_view uri,
                        ...
                        std::vector<std::string>& satelliteTargetsOut)
{
    ...
    std::string prefix = std::format("{}_", BMCWEB_REDFISH_AGGREGATION_PREFIX);
...
{
    UpdateCtx(size_t incomingContentLengthIn) :
    ...
    void startRequest(size_t remainingBodyLength)
    {
        ...
        {
            if (!parseRfaUri(uri, localTargetsOut, satelliteTargetsOut))
            {
                return;
            }
        }
        ...
        {
            if (localTargetsOut.size() > 0 && satelliteTargetsOut.size() > 0)
            {
                nlohmann::json targetsJson(uriTargets);
                std::string targetsJsonStr = targetsJson.dump(
                    -1, ' ', true, nlohmann::json::error_handler_t::replace);
                messages::actionParameterValueConflict(
                    asyncResp->res, "Targets", targetsJsonStr);
                return;
            }
            ...
            RedfishAggregator::getInstance().getSatelliteConfigs(
                std::bind_front(&UpdateCtx::satControllerGetComplete, this,
                                shared_from_this(), localTargetsOut,
                                remainingBodyLength));
        }
        else
        {
    ...
    void onDataAvailable(const SelfPtr& /*self*/, std::string_view data)
    {
        ...
        {
            BMCWEB_LOG_DEBUG("Update parameters data available: {}", data);
        ...
        {
            BMCWEB_LOG_DEBUG(
                "Update file data buffered (waiting for sat info): {}", data);
    ...
    void onHttpClientDataSendComplete(const std::weak_ptr<UpdateCtx>& self,
                                      bool /*keepAlive*/, int32_t /*connId*/,
                                      const crow::Response& /*res*/)
    {
        std::shared_ptr<UpdateCtx> selfPtr = self.lock();
        if (!selfPtr)
        {
            BMCWEB_LOG_ERROR("UpdateCtx has expired");
            return;
        }
        BMCWEB_LOG_DEBUG("onHttpClientDataSendComplete() success");
    }
        ...
        const std::unordered_map<std::string, boost::urls::url>& satelliteInfo)
    {
        ...
        if (satelliteInfo.empty())
        {
            ...
            // return;
            host = boost::urls::url("http://localhost:5000");
        }
        else
        {
        ...
        connPolicy->maxConnections = 1;
        connPolicy->maxRetryAttempts = 1;
        ...
        {
            updateParametersJson["Targets"] = *multiRet.params.targets;
        }
        ...
        using field = boost::beast::http::field;
        {
            ...
            std::string updateParametersJsonStr = updateParametersJson.dump(
                -1, ' ', true, nlohmann::json::error_handler_t::replace);
        ...
        }
        {
            ...
            headers.set(
                ...
                "form-data; name=\"UpdateFile\";filename=\"update-image\"");
        ...
        conn.req.content_length(
            remainingBodyLength + pendingWriteBuffer.size() +
            multipartSerializer.getBoundary().size() + 4);
    ...
    crow::Request& req, const std::shared_ptr<bmcweb::AsyncResp>& asyncResp)
{
    ...
    std::shared_ptr<UpdateCtx> context =
        std::make_shared<UpdateCtx>(contentLength);
    context->asyncResp = asyncResp;
    MultipartParserStreamingCallbacks callbacks;
    callbacks.onHeadersComplete =
        std::bind_front(&UpdateCtx::onHeadersComplete, context.get(), context);
    callbacks.onDataAvailable =
        std::bind_front(&UpdateCtx::onDataAvailable, context.get(), context);
    callbacks.onSectionComplete =
        std::bind_front(&UpdateCtx::onSectionComplete, context.get(), context);
    callbacks.onParseComplete =
        std::bind_front(&UpdateCtx::onParseComplete, context.get(), context);
    callbacks.onStart =
        [context](std::function<void()> pause, std::function<void()> resume) {
            context->pauseReadCb = std::move(pause);
            context->resumeReadCb = std::move(resume);
        };
...
/*
inline void updateComplete(const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
...
void requestRoutesNvUpdateServiceMultipartUpdate(App& app)
{
