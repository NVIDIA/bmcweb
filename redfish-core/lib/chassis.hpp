    ...
    namespace redfish
{
     ...
     *
     * Calls the Association endpoints on the path + "/storage" and add the link
     * of json["Links"]["Storage@odata.count"] =
     *    {"@odata.id", "/redfish/v1/Storage/" + resourceId}
     *
     * @return None.
     */
    inline void getStorageLink(
        ...
        const std::shared_ptr<bmcweb::AsyncResp>& asyncResp)
    {
        ...
        collection_util::getCollectionMembers(
            ...
            chassisInterfaces, "/xyz/openbmc_project/inventory");
    }
        ...
        const dbus::utility::DBusPropertiesMap& propertiesList)
    {
        ...
        // If there is an error or some enum we don't support just sit it to
        // Rack Mount
        ...
        const dbus::utility::MapperGetSubTreeResponse& subtree)
    {
                 ...
                 object : subtree)
        {
            ...
            if constexpr (BMCWEB_NVIDIA_OEM_PROPERTIES)
            {
                ...
                redfish::debug_token::getChassisDebugToken(asyncResp,
                                                           chassisId);
            }
            ...
            for (const auto& [connectionName, interfaces2] : connectionNames)
            {
                        ...
                        dbus::utility::getProperty<std::string>(
                            ...
                            [asyncResp,
                             chassisId](const boost::system::error_code& ec3,
                                ...
                                {
                                    BMCWEB_LOG_ERROR(
                                        "DBus response error for Version: {}",
                                        ec3);
                                    messages::internalError(asyncResp->res);
                                    return;
                                }
                ...
                dbus::utility::getAllProperties(
                    *crow::connections::systemBus, connectionName, path,
                    ...
                    [asyncResp, chassisId, path, connectionName,
                     interfaces2](const boost::system::error_code&,
                ...
                if constexpr (BMCWEB_NVIDIA_OEM_PROPERTIES)
                {
                    redfish::nvidia_chassis_utils::maybePopulateStaticPowerHint(asyncResp, path, interfaces2);
                }
                ...
                for (const auto& interface : interfaces2)
                {
                    ...
                    if constexpr (BMCWEB_NVIDIA_OEM_PROPERTIES)
                    {
                            ...
                            "xyz.openbmc_project.Inventory.Decorator.VendorInformation")
                        {
                            ...
                            redfish::nvidia_chassis_utils::
                                getOemCBCChassisAsset(asyncResp, connectionName,
                                                      path);
                        }
                    }
                }
                ...
                redfish::chassis_utils::getChassisLinksContainedBy(asyncResp,
                                                                   objPath);
                redfish::nvidia_chassis_utils::getPhysicalSecurityData(
                    asyncResp);
                // get network adapter
             ...
             temperature, hardwareWriteProtectEnable](
                         ...
                         object : subtree)
                {
                    ...
                    if (BMCWEB_NVIDIA_OEM_PROPERTIES)
                    {
                        ...
                        {
                            if (cpuClockFrequency && workloadFactor &&
                                temperature)
                            {
                                redfish::nvidia_chassis_utils::
                            ...
                            else
                            {
                                ...
                                {
                                    messages::propertyMissing(
                                        asyncResp->res, "TemperatureCelsius");
                                }
                            }
                        }
                    }
        ...
        const std::shared_ptr<bmcweb::AsyncResp>& asyncResp)
    {
        ...
        dbus::utility::getSubTreePaths(
            ...
            [asyncResp](const boost::system::error_code& ec,
                ...
                if (ec)
                {
                    BMCWEB_LOG_ERROR("[mapper] Bad D-Bus request error: {}",
                                     ec);
                    messages::internalError(asyncResp->res);
                    return;
                }
    ...
    inline void powerCycle(const std::shared_ptr<bmcweb::AsyncResp>& asyncResp)
    {
        crow::connections::systemBus->async_method_call(
            [asyncResp](const boost::system::error_code& ec,
                ...
                std::string objectPath =
                    "/xyz/openbmc_project/state/host_system0";
                ...
                crow::connections::systemBus->async_method_call(
                    [asyncResp,
                     objectPath](const boost::system::error_code& ec2,
                        ...
                        {
                            BMCWEB_LOG_DEBUG(
                                "[mapper] Bad D-Bus request error: ", ec2);
                            messages::internalError(asyncResp->res);
                            return;
                        }
    ...
    inline void requestRoutesChassisResetAction(App & app)
    {
        BMCWEB_ROUTE(app, "/redfish/v1/Chassis/<str>/Actions/Chassis.Reset/")
            ...
            .methods(boost::beast::http::verb::post)(std::bind_front(
                handleChassisResetActionInfoPost, std::ref(app)));
        if constexpr (BMCWEB_HOST_AUXPOWER_FEATURES)
        {
            BMCWEB_ROUTE(
                app,
    ...
    inline void requestRoutesChassisResetActionInfo(App & app)
    {
        BMCWEB_ROUTE(app, "/redfish/v1/Chassis/<str>/ResetActionInfo/")
            ...
            .methods(boost::beast::http::verb::get)(std::bind_front(
                handleChassisResetActionInfoGet, std::ref(app)));
        if constexpr (BMCWEB_HOST_AUXPOWER_FEATURES)
        {
            BMCWEB_ROUTE(
                app,
