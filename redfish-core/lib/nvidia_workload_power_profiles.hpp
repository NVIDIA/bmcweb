...
inline void getProcessorWorkloadPowerInfo(
        ...
        [processorId, aResp{std::move(aResp)}](
            ...
            for (const auto& [path, object] : subtree)
            {
                ...
                aResp->res.jsonValue["Name"] =
                    processorId + " Workload Power Profile";

                std::string presetProfileURI = workLoadPowerURI;
                presetProfileURI += "/Profiles";
                aResp->res.jsonValue["Profiles"]["@odata.id"] =
                    presetProfileURI;
                ...
                enableProfilesURI +=
                    "/Actions/NvidiaWorkloadPower.EnableProfiles";
                aResp->res
                    .jsonValue["Actions"]["#NvidiaWorkloadPower.EnableProfiles"]
        ...
        [processorId, aResp{std::move(aResp)}](
            ...
            for (const auto& [path, object] : subtree)
            {
                ...
                profileCollectionURI +=
                    "/Oem/Nvidia/WorkloadPowerProfile/Profiles";
                aResp->res.jsonValue["@odata.type"] =
                ...
                for (const auto& [serv, _] : object)
                {
                    ...
                    nvidia_async_operation_utils::
                        doGenericCallAsyncAndGatherResult<int>(
                            ...
                            path, "com.nvidia.PowerProfile.ProfileInfoAsync",
                            "EnablePresetProfile",
                            [asyncResp, profileMask](
                                ...
                                {
                                    BMCWEB_LOG_DEBUG(
                                        "EnablePresetProfile Succeeded");
                                    messages::success(asyncResp->res);
                                    return;
                                }
                                BMCWEB_LOG_ERROR(
                                    "EnablePresetProfile Throws error {}",
                                    status);
...
inline void postEnableWorkLoadPowerProfile(
    std::shared_ptr<bmcweb::AsyncResp> aResp, const std::string& processorId,
                    ...
                    nvidia_async_operation_utils::
                        doGenericCallAsyncAndGatherResult<int>(
                            ...
                            [asyncResp, profileMask](
                                ...
                                {
                                    BMCWEB_LOG_DEBUG(
                                        "DisablePresetProfile Succeeded");
                                    messages::success(asyncResp->res);
                                    return;
                                }
                                BMCWEB_LOG_ERROR(
                                    "DisablePresetProfile Throws error {}",
                                    status);
                                messages::internalError(asyncResp->res);
                            },
                            profileMask);
...
inline void postDisableWorkLoadPowerProfile(
    std::shared_ptr<bmcweb::AsyncResp> aResp, const std::string& processorId,
    std::vector<uint8_t>& profileMask)
{
    BMCWEB_LOG_DEBUG("Get available system processor resource");
    ...
    BMCWEB_ROUTE(app, "/redfish/v1/Systems/" +
        ...
        .methods(boost::beast::http::verb::get)(
            [&app](const crow::Request& req,
                ...
                validateProcessorAndGetWorkloadPowerInfo(asyncResp,
                                                         processorId);
            });
    ...
    BMCWEB_ROUTE(
        ...
        .methods(boost::beast::http::verb::get)(
            [&app](const crow::Request& req,
                ...
                getProcessorWorkloadPowerProfileCollectionData(asyncResp,
                                                               processorId);
            });
}
