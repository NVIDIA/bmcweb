    ...
    namespace redfish
{
    ...
    inline std::optional<std::string> convertDriveOperation(
        const std::string& op)
    {
        ...
        sdbusplus::asio::getProperty<std::string>(
            *crow::connections::systemBus, connectionName, path,
            ...
            [asyncResp, connectionName, path](
                const boost::system::error_code& ec4, const std::string& sw) {
                if (ec4)
                {
        ...
        sdbusplus::asio::getProperty<uint8_t>(
            *crow::connections::systemBus, connectionName, path,
            ...
            [asyncResp, operationName](const boost::system::error_code& ec,
                ...
                asyncResp->res.jsonValue["Operations"] =
                ...
                asyncResp->res.jsonValue["Operations"].emplace_back(
                    std::move(obj));
            });
    }
        ...
        const std::string& connectionName, const std::string& path)
    {
        sdbusplus::asio::getProperty<std::string>(
            *crow::connections::systemBus, connectionName, path,
            ...
            [asyncResp, connectionName, path](
                const boost::system::error_code& ec5, const std::string& op) {
                if (ec5)
                {
                    BMCWEB_LOG_ERROR("fail to get drive progress");
                    return;
    ...
    inline void requestRoutesStorageCollection(App & app)
    {
        BMCWEB_ROUTE(app, "/redfish/v1/Systems/<str>/Storage/")
            ...
            .methods(boost::beast::http::verb::get)(std::bind_front(
                handleSystemsStorageCollectionGet, std::ref(app)));
    ...
    inline std::optional<drive::MediaType> convertDriveType(
        std::string_view type)
    {
        ...
        if (type ==
            "xyz.openbmc_project.Inventory.Item.Drive.DriveType.Unknown")
        {
            return std::nullopt;
        }
    ...
    {
        if (proto ==
            "xyz.openbmc_project.Inventory.Item.Drive.DriveProtocol.SAS")
        {
            return protocol::Protocol::SAS;
        }
        if (proto ==
            "xyz.openbmc_project.Inventory.Item.Drive.DriveProtocol.SATA")
        {
        ...
        if (proto ==
            "xyz.openbmc_project.Inventory.Item.Drive.DriveProtocol.FC")
        {
            ...
            [asyncResp](
                     ...
                     propertiesList)
                {
                    ...
                    else if (propertyName == "EncryptionStatus")
                    {
                        ...
                        {
                            BMCWEB_LOG_ERROR(
                                "Illegal property: EncryptionStatus");
                            messages::internalError(asyncResp->res);
                            return;
                        }
                    }
                    ...
                    else if (propertyName == "FormFactor")
                    {
                        ...
                        asyncResp->res.jsonValue["DriveFormFactor"] =
                            *formFactor;
                    }
                }
        ...
        dbus::utility::getSubTree(
            ...
            [req, asyncResp, driveId, sanitizeType,
             owPass](const boost::system::error_code& ec4,
                ...
                const dbus::utility::MapperServiceMap& connNames =
                    drive->second;
        ...
        const std::string& systemName)
    {
        ...
        asyncResp->res.jsonValue["@odata.id"] =
            "/redfish/v1/Systems/" +
            std::string(BMCWEB_REDFISH_SYSTEM_URI_NAME) + "/Storage/1/Drives/";
        ...
        const dbus::utility::MapperGetSubTreeResponse& subtree)
    {
        ...
        for (const auto& [path, connectionNames] : subtree)
        {
            ...
            for (const auto& [connectionName, connInterfaces] : connectionNames)
            {
                addAllDriveInfo(asyncResp, connectionName, path,
                                connInterfaces);
            }
        }
    }
    ...
    /**
     * This URL will show the drive interface for the specific drive in the
     * chassis
     */
    inline void requestRoutesChassisDriveName(App & app)
    {
        ...
        BMCWEB_ROUTE(
            app, "/redfish/v1/Chassis/<str>/Drives/<str>/SanitizeActionInfo/")
            .privileges(redfish::privileges::getDrive)
            .methods(boost::beast::http::verb::get)(std::bind_front(
        ...
        dbus::utility::getSubTreePaths(
            ...
            [asyncResp](const boost::system::error_code& ec,
                        const dbus::utility::MapperGetSubTreePathsResponse&
                            controllerList) {
                populateStorageControllerCollection(asyncResp, ec,
                                                    controllerList);
            });
    }
        ...
        const std::string& systemName, const std::string& controllerId)
    {
        ...
        dbus::utility::getSubTree(
            ...
            [asyncResp, controllerId](
                const boost::system::error_code& ec,
                const dbus::utility::MapperGetSubTreeResponse& subtree) {
                getStorageControllerHandler(asyncResp, controllerId, ec,
                                            subtree);
            });
    }
    ...
    {
        BMCWEB_ROUTE(app,
                     "/redfish/v1/Systems/<str>/Storage/1/Controllers/<str>/")
            .privileges(redfish::privileges::getStorageController)
            .methods(boost::beast::http::verb::get)(std::bind_front(
                handleSystemsStorageControllerGet, std::ref(app)));
    }
