    ...
    crow::connections::systemBus->async_method_call(
        ...
        for (const auto& property : properties)
        {
                     ...
                     "PrimaryFloorTargetWindowMultiplierApplied")
            {
                ...
                aResp->res.jsonValue["AdminOverrideActiveMask"]
                                    ["PFTWindowMultiplierSettingApplied"] = *value;
            }
    ...
    crow::connections::systemBus->async_method_call(
        ...
        for (const auto& property : properties)
        {
            ...
            else if (property.first == "FloorWindowMultiplier")
            {
                ...
                aResp->res.jsonValue["FloorWindowMultiplierPeriod"] = *value;
            }
            ...
            crow::connections::systemBus->async_method_call(
                ...
                for (const std::string& profilePath : *data)
                {
                    ...
                    crow::connections::systemBus->async_method_call(
                        ...
                        for (const auto& [path, object] : subtree)
                        {
                            BMCWEB_LOG_DEBUG("DBUS path {}", profilePath);
                            if (path != profilePath)
                            {
                                continue;
                            }
                        ...
                        for (const auto& [service, interfaces] : object)
                        {
                            ...
                            nvidia_async_operation_utils::
                                doGenericSetAsyncAndGatherResult(
                                    ...
                                    profilePath,
                                    "com.nvidia.PowerSmoothing.AdminPowerProfile",
                                    propName, std::variant<double>(propValue),
                                    nvidia_async_operation_utils::
                                        PatchGenericCallback{aResp});
                        }
                    },
                               ...
                               std::string propName, double propValue)
{
    ...
    crow::connections::systemBus->async_method_call(
        ...
        for (const auto& [path, object] : subtree)
        {
            BMCWEB_LOG_ERROR("path : {}", boost::ends_with(path, processorId));
            if (!boost::ends_with(path, processorId))
            {
                continue;
            }
...
inline void requestRoutesProcessorPowerSmoothingAdminProfile(App& app)
{
    ...
    BMCWEB_ROUTE(
        ...
        if (!redfish::json_util::readJsonAction(
                ...
                "PFAWindowMultiplier", primaryFloorActivationWindowMultiplier,
                "PFTWindowMultiplier", primaryFloorTargetWindowMultiplier,
                "PFAOffsetWatts", primaryFloorActivationOffset))
        {
            return;
        }
        if (tmpFloorPercent)
