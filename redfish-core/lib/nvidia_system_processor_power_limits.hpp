...
{
    std::string objectPath;
    if (dimension == PowerLimitDimension::Total)
    {
        objectPath = gpuPaths.gpuObjectPath;
    }
    else
    {
        if (!gpuPaths.basePowerLimitPath.has_value())
        {
            recordOutcome(ctx,
                          SetOutcome{.processorId = gpuPaths.processorId,
                                     .dimension = dimension,
                                     .status = "Unavailable",
                                     .message = "Base Power Limit not found"});
            return;
        }
        objectPath = *gpuPaths.basePowerLimitPath;
    }
