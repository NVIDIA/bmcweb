...
static const std::map<const DotMctpVdmUtilCommand, const std::string>
    commandsMap{{DotMctpVdmUtilCommand::CAKInstall, "cak_install"},
                      ...
                      const std::vector<uint8_t>& data)
{
    ...
    auto resultHandler = [asyncResp](const std::string& output) {
        ...
        try
        {
            ...
            {
                dotCompletionCode = std::stoi(dotCompletionCodeHexStr, nullptr,
                                              16);
            }
        }
        catch (...)
