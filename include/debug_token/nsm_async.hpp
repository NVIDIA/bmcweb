...
class Handler : public std::enable_shared_from_this<Handler>
{
  public:
                               ...
                               Argument arg, ResultCallback callback)
    {
        ...
        {
            MakeSharedHelper(Operation op, Argument arg,
                             ResultCallback callback) :
                Handler(op, arg, std::move(callback))
            {}
        };
  ...
  private:
     ...
     *
     * @param callback The callback function to be called when the async value
     * is obtained
     */
    template <typename T, typename U>
    void resultHandler(U&& callback)
    {
        sdbusplus::asio::getProperty<T>(
            *crow::connections::systemBus, asyncObjectService, asyncObjectPath,
            std::string(asyncValueIntf), "Value", std::forward<U>(callback));
    }
                        ...
                        const RequestType& dbusFd)
    {
        ...
        switch (nsmReq->status)
        {
            case NsmDebugTokenChallengeQueryStatus::OK:
                result = std::make_tuple(EndpointState::RequestAcquired,
                                         request);
                break;
            case NsmDebugTokenChallengeQueryStatus::TokenAlreadyApplied:
                result = std::make_tuple(EndpointState::TokenInstalled,
                                         request);
                break;
            ...
            default:
                BMCWEB_LOG_ERROR("NSM token request - status: {}",
                                 nsmReq->status);
                result = std::make_tuple(EndpointState::Error,
                                         std::monostate());
                break;
        }
    }
