...
Item::Item(uint16_t type, const std::vector<uint8_t>& input)
{
    ...
    header->size = htole16(static_cast<uint16_t>(input.size()));
...
void Structure::add(uint16_t type, const std::vector<uint32_t>& input)
{
    ...
    try
    {
        ...
        for (const auto& value : input)
        {
            ...
            std::memcpy(bytes.data(), &leValue, sizeof(leValue));
            inputU8.insert(inputU8.end(), bytes.begin(), bytes.end());
        }
...
void Structure::add(uint16_t type, uint16_t value)
{
    ...
    std::memcpy(bytes.data(), &leValue, sizeof(leValue));
    add(type, std::vector<uint8_t>(bytes.begin(), bytes.end()));
}
