...
Item::Item(const std::span<const uint8_t> input)
{
    ...
    data = std::vector<uint8_t>(
        ...
        input.begin() +
            static_cast<std::ptrdiff_t>(sizeof(ItemHeader) + dataLength));
}
...
T Item::getValue() const
{
    ...
    for (size_t i = 0; i < data.size(); i += sizeof(typename T::value_type))
    {
        ...
        std::memcpy(value.get(), &dataSpan[i], sizeof(typename T::value_type));
        if (std::is_same_v<T, std::vector<uint16_t>>)
        {
            result.push_back(
                static_cast<typename T::value_type>(le16toh(*value)));
        }
        if (std::is_same_v<T, std::vector<uint32_t>>)
        {
            result.push_back(
                static_cast<typename T::value_type>(le32toh(*value)));
        }
        if (std::is_same_v<T, std::vector<uint64_t>>)
        {
            result.push_back(
                static_cast<typename T::value_type>(le64toh(*value)));
        }
    }
    return result;
}
...
T Item::getValue() const
{
    ...
    {
        return static_cast<T>(le16toh(*result));
    }
    if (std::is_same_v<T, uint32_t>)
    {
        return static_cast<T>(le32toh(*result));
    }
    if (std::is_same_v<T, uint64_t>)
    {
        return static_cast<T>(le64toh(*result));
    }
    return *result;
}
...
void Structure::decode(const std::vector<uint8_t>& input)
{
    ...
    auto currentItr =
        input.begin() + static_cast<std::ptrdiff_t>(sizeof(StructureHeader));
    while (currentItr < input.end())
    {
        try
        {
            ...
            currentItr += static_cast<std::ptrdiff_t>(elementSize);
        }
