...
class Handler : public std::enable_shared_from_this<Handler>
{
  public:
                               ...
                               ResultCallback callback)
    {
        ...
        {
            MakeSharedHelper(const std::vector<Eid>& eids,
                             ResultCallback callback) :
                Handler(eids, std::move(callback))
            {}
        };
  ...
  private:
    Handler(const std::vector<Eid>& eids, ResultCallback cb) :
        eids(eids), callback(cb)
    {}
    ...
    void run()
    {
        createTimer(static_cast<int>(eids.size() *
                                     (2 * vdmTokenStatusQueryTimeout + 1)));
