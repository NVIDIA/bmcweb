...
TEST(MultipartTestChunked, TestGoodMultipartParserChunked)
{
    ...
    {
        MultipartParser parser(goodMultipartParserBody.size());
...
TEST_F(MultipartTest, callbacksAreCalledCorrectly)
{
    ...
    std::string dataStringOut;
    bool sectionCompleteCalled = false;
    ...
    EXPECT_EQ(parser.boundary, "\r\n----XX");
    EXPECT_TRUE(headersCompleteCalled);
    EXPECT_TRUE(sectionCompleteCalled);
    EXPECT_TRUE(parseCompleteCalled);

    EXPECT_EQ(dataStringOut, "Data1\r\n"
                             "----XX-abc-\r\n"
                             "StillData1");
